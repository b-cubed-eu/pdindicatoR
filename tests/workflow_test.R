...
plot(tree, cex=0.45, ladderize = TRUE)
library(ggtree)
ggtree(tree) + geom_tiplab(size = 2, align = TRUE)
...
# TO DO: generate info/warning message with how many specieskeys could not be matched with a tree leaf label (count(ott_id is NA)
...
aggr_cube <- simpl_cube %>% group_by(eeaCellCode) %>% summarize(speciesKeys = list(speciesKey), ott_ids = list(ott_id), names = list(unique_name)) %>%
...
# Adapt in order to account for year

head(mcube)
simpl_cube <- mcube[,c("year","eeaCellCode","speciesKey","ott_id", "unique_name")]
simpl_cube$eeaCellCode <- factor(simpl_cube$eeaCellCode)
simpl_cube$year <- factor(simpl_cube$year)
simpl_cube <- simpl_cube %>% arrange(eeaCellCode, year)
simpl_cube
aggr_cube <- simpl_cube %>% group_by(eeaCellCode, year) %>% summarize(speciesKeys = list(speciesKey), ott_ids = list(ott_id), names = list(unique_name))
print(aggr_cube, n=100)
...
head(aggr_cube)
aggr_cube[[1,4]]
aggr_cube <- simpl_cube %>% arrange(eeaCellCode)
